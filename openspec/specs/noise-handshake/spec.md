...
**MODIFICATION**: The 3-emoji SAS SHALL be derived from the Noise transport key after XX handshake completion, using `SHA-256(encryption_key) → 18 bits → 3 × 6-bit emoji indices`.
...
- **WHEN** the extension reconnects after browser restart using WebAuthn PRF silent re-auth
- **THEN** the PRF-derived 32-byte key SHALL be used as `localStaticKey` for the IK handshake
- **AND** the handshake SHALL complete without user interaction
