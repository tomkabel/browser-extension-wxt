...
- [x] 1.1 Implement `challengeDerivation.ts` in `lib/webauthn/`: canonical TLV serialization of `Version(1) || zkTLS_Proof_Length(2) || zkTLS_Proof(var) || Origin_Length(2) || Origin(var) || Control_Code_Length(1) || Control_Code(4) || Session_Nonce(32) || Padding(var)` then SHA-256 hash
...
- [x] 1.3 Unit test: TLV serialization roundtrip — serialize then parse produces identical components
- [x] 1.4 Unit test: same inputs produce same challenge hash (determinism)
- [x] 1.5 Unit test: different session nonce produces different challenge (nonce binding)
- [x] 1.6 Unit test: padding is canonical (deterministic to next 32-byte boundary, zero-filled)
...
- [x] 4.1 Implement `ChallengeVerifier.java` in Android Vault: parse TLV-serialized components, verify zkTLS proof first, recompute SHA-256 hash
...
- [x] 4.5 Unit test: challenge verification passes with valid inputs
- [x] 4.6 Unit test: challenge verification fails on mismatched origin, control code, or nonce
...
- [x] 6.1 Create `AuthPanel` updates: after zkTLS proof received, show transaction context (origin, control code) before biometric prompt
- [x] 6.2 Show "Waiting for biometric verification..." during `navigator.credentials.get()` invocation
- [x] 6.3 On success, show "Transaction verified via Challenge-Bound WebAuthn" with assertion details
