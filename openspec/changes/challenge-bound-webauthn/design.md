...
- Any tampering with origin, control code, or zkTLS proof invalidates the assertion
- The user's PC biometric cannot be repurposed for a different transaction

## Goals / Non-Goals

**Goals:**
- SHA-256 challenge derivation from zkTLS proof, origin, control code, and nonce
- WebAuthn assertion with derived challenge via `navigator.credentials.get()`
- Challenge recomputation on Android after zkTLS proof verification
...
- Serialization of challenge components into canonical binary format
...
│ Version (1 byte) = 0x01                        │
├───────────────────────────────────────────────┤
│ zkTLS_Proof_Length (2 bytes, big-endian)       │
│ zkTLS_Proof (variable, up to 4096 bytes)       │
├───────────────────────────────────────────────┤
...
The Java `ChallengeVerifier` performs these steps in order:
