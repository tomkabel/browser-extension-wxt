...
- **AND** if either check fails, SHALL wait 500ms and retry up to 2 times after the initial attempt (3 total attempts)
...
- **AND** after `maxRetries` retries after the initial attempt (maxRetries + 1 total attempts), SHALL abort and report failure to Orchestrator
