...
- **AndroidManifest.xml**: `SYSTEM_ALERT_WINDOW` permission for overlay. `FOREGROUND_SERVICE` + `FOREGROUND_SERVICE_SPECIAL_USE` permissions (API 34+), `android:foregroundServiceType="specialUse"` on the gate service. `VIBRATE` permission.
