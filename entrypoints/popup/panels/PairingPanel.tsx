...
export const PairingPanel = forwardRef<HTMLHeadingElement>(
  function PairingPanel(_props, headingRef) {
    ...
    const pairingUrlRef = useRef<string | null>(null);
    ...
    const startPairing = useCallback(async () => {
      ...
      try {
        ...
        if (publicKeyBytes && nonceRef.current) {
          ...
          pairingUrlRef.current = buildPairingUrl(code, nonceRef.current, commitment);
        }
      } catch (err) {
        setPairingState('error');
    ...
    useEffect(() => {
      ...
      if (pairingState === 'displaying_qr' && canvasRef.current && sasCode) {
        const pairingUrl = pairingUrlRef.current ?? buildPairingUrl(sasCode);
    ...
    const remainingSeconds = Math.max(
      0,
      Math.ceil((SAS_TTL_MS - (Date.now() - generatedAt)) / 1000),
    );
      ...
      if (
        ...
        return (
          <div className="p-4 bg-white rounded-lg border text-center">
            ...
            <p className="text-sm text-gray-600 mb-4">Check your network and try again.</p>
    ...
    if (pairingState === 'awaiting_sas_confirmation') {
      ...
      return (
        <div className="p-4 bg-white rounded-lg border">
          ...
          <p className="text-sm text-gray-600 mb-4">Do these symbols match your phone screen?</p>
